export class ModalWebRtcClient extends EventTarget {
    ...
    async startStreaming() {
        this.peerID = this.peerID || this.generateShortUUID();
        this.updateStatus('Connecting to AI Buddy in the cloud (first connect can take ~20s)...');
    ...
    async negotiate() {
        try {
            ...
            // Add local stream to peer connection (mic audio = sendrecv, so the
            // server can also send synthesized speech back on the same m-line)
            this.localStream.getTracks().forEach(track => {
                console.log('Adding track:', track);
                this.peerConnection.addTrack(track, this.localStream);
            });

            // We RECEIVE the avatar video — the offer must carry a video m-line
            // for the server to attach its track to, so add a recvonly transceiver.
            this.peerConnection.addTransceiver('video', { direction: 'recvonly' });

            // Data channel for server -> client status & caption events.
            const dc = this.peerConnection.createDataChannel('events');
            dc.onmessage = (e) => {
                try {
                    this.dispatchEvent(new CustomEvent('serverEvent', { detail: JSON.parse(e.data) }));
                } catch (_) {}
            };
            this.dataChannel = dc;
